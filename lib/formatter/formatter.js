...
function createFormatter(options, stream) {
    ...
    var delimiter = options.delimiter || ",",
        ESCAPE_REGEXP = new RegExp("[" + delimiter  + "']", "gmi"),
        NEWLINE_REGEXP = new RegExp("[" + escape(options.rowDelimiter || LINE_BREAK) + "']", "gm"),
        QUOTE = options.quote || '"',
    ...
    function escapeField(field, index, isHeader) {
        ...
        if ((escape = field.indexOf(QUOTE) !== -1)) {
            field = field.replace(REPLACE_REGEXP, ESCAPE + QUOTE).replace(NEWLINE_REGEXP, " ");
            escape = true;
        } else {
